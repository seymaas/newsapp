...
//add new data in firebase for favorites||unfavorites||readlater collection
  Future addData(bool status,String title,description,image,source,url,collection,{flushBarerrorMsg,context}) {
    CollectionReference collectionReference = databaseReference.collection(collection);
...
//delete selected data in firebase for favorites||unfavorites||readlater collection
  Future<void> deleteData(String url,collection,{flushBarerrorMsg,context}) async {
    QuerySnapshot snap = await FirebaseFirestore.instance.collection(collection).get();
    if(collection == "readLater") showFlushbarFun(Colors.teal[200], flushBarerrorMsg, context);
